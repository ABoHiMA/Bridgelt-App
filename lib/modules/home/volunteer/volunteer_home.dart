    ...
    return BlocConsumer<AppCubit, AppStates>(
      ...
      builder: (context, state) {
        ...
        return ConditionalBuilder(
          ...
          fallback: (context) => cubitApp.listOfVCases.isNotEmpty
              ? ListView.separated(
                  shrinkWrap: true,
                  ...
                  itemBuilder: (context, index) => cubitApp.isFilter
                      ? spCases(
                          context,
                          index,
                          cubitApp.listOfFilteredVCases[index],
                        )
                      : spCases(
                          context,
                          index,
                          cubitApp.listOfVCases[index],
                        ),
                  separatorBuilder: (context, index) => const SizedBox(),
                  itemCount: cubitApp.isFilter
                      ? cubitApp.listOfFilteredVCases.length
                      : cubitApp.listOfVCases.length,
                )
              : Center(
                  child: txt(
                    context: context,
...
Widget spCases(
  ...
  Cases model,
    ...
    Padding(
      ...
      child: Card(
        // color: const Color(0x00FEEDF4).withOpacity(0.69),
        surfaceTintColor: bg,
        shadowColor: bg,
        elevation: 3,
                      ...
                      return AlertDialog(
                        ...
                        actions: <Widget>[
                          ElevatedButton(
                            onPressed: () {
                              AppCubit.get(context)
                                  .getSpecialNeedCaseChat(otheruid: model.uId!);
                            },
